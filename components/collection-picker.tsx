...
export function CollectionPicker({ target, visible, onClose }: { target: { type: 'adventure' | 'curiosity'; id: string }; visible: boolean; onClose: () => void }) {
  ...
  return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
    ...
    <View style={styles.sheet}>
      ...
      {loading ? <ActivityIndicator color="#B86F4B" /> : collections.map((collection) => {
...
const styles = StyleSheet.create({
  ...
  sheet: { maxHeight: '72%', backgroundColor: '#173523', borderTopWidth: 1, borderColor: '#55775B', padding: 20, paddingBottom: 36 },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 },
  eyebrow: { color: '#B86F4B', fontSize: 10, fontWeight: '900', letterSpacing: 1.4 },
  title: { color: '#F4E9D6', fontSize: 24, fontWeight: '900', marginTop: 4 },
  close: { color: '#F4E9D6', fontSize: 34, lineHeight: 36 },
  row: { minHeight: 64, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderTopWidth: 1, borderColor: '#31513A', paddingVertical: 10 },
  rowTitle: { color: '#F4E9D6', fontSize: 15, fontWeight: '800' },
  rowMeta: { color: '#789086', fontSize: 11, marginTop: 3 },
  check: { width: 34, height: 34, color: '#B86F4B', fontSize: 22, lineHeight: 32, textAlign: 'center', borderWidth: 1, borderColor: '#748D73' },
  checkSelected: { color: '#0B1710', backgroundColor: '#B86F4B' },
  createRow: { flexDirection: 'row', gap: 8, marginTop: 18 },
  input: { flex: 1, height: 48, color: '#F4E9D6', borderWidth: 1, borderColor: '#748D73', paddingHorizontal: 12 },
  createButton: { height: 48, justifyContent: 'center', backgroundColor: '#B86F4B', paddingHorizontal: 18 },
  createButtonText: { color: '#0B1710', fontSize: 13, fontWeight: '900' },
});
