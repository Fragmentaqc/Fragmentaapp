...
export default function ResetPasswordScreen() {
  ...
  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><Text style={styles.eyebrow}>SÉCURITÉ</Text><Text style={styles.title}>Nouveau mot de passe</Text><Text style={styles.label}>Mot de passe</Text><TextInput value={password} onChangeText={setPassword} secureTextEntry style={styles.input} placeholder="Au moins 8 caractères" placeholderTextColor="#63766D" /><Text style={styles.label}>Confirmer</Text><TextInput value={confirmation} onChangeText={setConfirmation} secureTextEntry style={styles.input} placeholder="Répète le mot de passe" placeholderTextColor="#63766D" /><Pressable style={[styles.button, saving && styles.disabled]} disabled={saving} onPress={() => void savePassword()}>{saving ? <ActivityIndicator color="#071310" /> : <Text style={styles.buttonText}>Enregistrer</Text>}</Pressable></KeyboardAvoidingView></SafeAreaView>;
}

const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#071310' }, container: { flex: 1, padding: 22, justifyContent: 'center' }, center: { flex: 1, padding: 24, alignItems: 'center', justifyContent: 'center' }, eyebrow: { color: '#62E6B1', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 }, title: { color: '#F3FFF9', fontSize: 28, fontWeight: '900', textAlign: 'center', marginBottom: 20 }, description: { color: '#8FA69B', textAlign: 'center', lineHeight: 20 }, label: { color: '#DFFFF2', fontSize: 13, fontWeight: '900', marginTop: 14, marginBottom: 8 }, input: { minHeight: 56, borderRadius: 16, borderWidth: 1, borderColor: '#1D4538', backgroundColor: '#0C1C17', color: '#F3FFF9', paddingHorizontal: 16 }, button: { minHeight: 58, alignItems: 'center', justifyContent: 'center', borderRadius: 18, backgroundColor: '#62E6B1', marginTop: 24 }, buttonText: { color: '#071310', fontSize: 14, fontWeight: '900' }, secondary: { borderRadius: 16, borderWidth: 1, borderColor: '#28634F', padding: 14, marginTop: 20 }, secondaryText: { color: '#62E6B1', fontWeight: '900' }, disabled: { opacity: 0.5 } });
