...
export default function ResetPasswordScreen() {
  ...
  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><Text style={styles.eyebrow}>SÉCURITÉ</Text><Text style={styles.title}>Nouveau mot de passe</Text><Text style={styles.label}>Mot de passe</Text><TextInput value={password} onChangeText={setPassword} secureTextEntry style={styles.input} placeholder="Au moins 8 caractères" placeholderTextColor="#B8B59E" /><Text style={styles.label}>Confirmer</Text><TextInput value={confirmation} onChangeText={setConfirmation} secureTextEntry style={styles.input} placeholder="Répète le mot de passe" placeholderTextColor="#B8B59E" /><Pressable style={[styles.button, saving && styles.disabled]} disabled={saving} onPress={() => void savePassword()}>{saving ? <ActivityIndicator color="#173E28" /> : <Text style={styles.buttonText}>Enregistrer</Text>}</Pressable></KeyboardAvoidingView></SafeAreaView>;
}

const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#173E28' }, container: { flex: 1, padding: 22, justifyContent: 'center' }, center: { flex: 1, padding: 24, alignItems: 'center', justifyContent: 'center' }, eyebrow: { color: '#E9576F', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 }, title: { color: '#F5E6C8', fontSize: 28, fontWeight: '900', textAlign: 'center', marginBottom: 20 }, description: { color: '#D8CFBA', textAlign: 'center', lineHeight: 20 }, label: { color: '#FFF1D6', fontSize: 13, fontWeight: '900', marginTop: 14, marginBottom: 8 }, input: { minHeight: 56, borderRadius: 0, borderWidth: 1, borderColor: '#356F43', backgroundColor: '#245A35', color: '#F5E6C8', paddingHorizontal: 16 }, button: { minHeight: 58, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#E9576F', marginTop: 24 }, buttonText: { color: '#173E28', fontSize: 14, fontWeight: '900' }, secondary: { borderRadius: 0, borderWidth: 1, borderColor: '#6D9F6B', padding: 14, marginTop: 20 }, secondaryText: { color: '#E9576F', fontWeight: '900' }, disabled: { opacity: 0.5 } });
