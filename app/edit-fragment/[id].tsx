...
export default function EditFragmentScreen() {
  ...
  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
    ...
    <Pressable style={styles.save} onPress={() => void save()} disabled={saving}>{saving ? <ActivityIndicator color="#071A1C" /> : <Text style={styles.saveText}>Enregistrer</Text>}</Pressable>
...
const styles = StyleSheet.create({
  ...
  back: { color: '#C99A2E', fontSize: 15, fontWeight: '800', marginBottom: 20 }, eyebrow: { color: '#C99A2E', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 }, heading: { color: '#F4EBD8', fontSize: 28, fontWeight: '900', marginTop: 6 },
  label: { color: '#FFF6E5', fontSize: 13, fontWeight: '800', marginTop: 18, marginBottom: 8 }, input: { minHeight: 54, borderRadius: 0, borderWidth: 1, borderColor: '#265F63', backgroundColor: '#10363A', color: '#F4EBD8', padding: 15 }, area: { minHeight: 150 },
  row: { flexDirection: 'row', gap: 9 }, choice: { flex: 1, alignItems: 'center', borderRadius: 0, borderWidth: 1, borderColor: '#3C7475', padding: 14 }, active: { backgroundColor: '#4B8180' }, choiceText: { color: '#F4EBD8', fontWeight: '800' },
  note: { color: '#B8C8C2', fontSize: 11, marginTop: 15 }, save: { minHeight: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#C99A2E', marginTop: 24 }, saveText: { color: '#071A1C', fontSize: 15, fontWeight: '900' },
});
