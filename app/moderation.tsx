...
type Report = { id: string; reporter_id: string; adventure_id: string | null; curiosity_id: string | null; reported_user_id: string | null; reason: string; details: string; status: ReportStatus; created_at: string };
...
export default function ModerationScreen() {
  ...
  const load = useCallback(async () => {
    ...
    setAuthorized(true);
    const { data, error } = await supabase.from('reports').select('id, reporter_id, adventure_id, curiosity_id, reported_user_id, reason, details, status, created_at').order('created_at', { ascending: true });
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 20, borderWidth: 1, borderColor: '#285345', backgroundColor: '#0C1C17', padding: 16, marginTop: 12 }, cardTop: { flexDirection: 'row', justifyContent: 'space-between', gap: 10 }, reason: { flex: 1, color: '#F3FFF9', fontSize: 15, fontWeight: '900' }, status: { color: '#071310', backgroundColor: '#E9B949', borderRadius: 999, paddingHorizontal: 8, paddingVertical: 5, fontSize: 8, fontWeight: '900' }, date: { color: '#63766D', fontSize: 10, marginTop: 7 }, details: { color: '#B7C9C1', fontSize: 13, lineHeight: 20, marginTop: 12 }, noDetails: { color: '#63766D', fontSize: 11, fontStyle: 'italic', marginTop: 12 }, target: { borderRadius: 13, backgroundColor: '#173D31', padding: 12, marginTop: 13 }, targetText: { color: '#62E6B1', fontSize: 11, fontWeight: '900' }, actions: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }, review: { borderRadius: 12, backgroundColor: '#28634F', padding: 10 }, reviewText: { color: '#FFF', fontSize: 10, fontWeight: '900' }, resolve: { borderRadius: 12, backgroundColor: '#62E6B1', padding: 10 }, resolveText: { color: '#071310', fontSize: 10, fontWeight: '900' }, dismiss: { borderRadius: 12, borderWidth: 1, borderColor: '#7B3535', padding: 10 }, dismissText: { color: '#FFB8B8', fontSize: 10, fontWeight: '900' }, primary: { minHeight: 50, justifyContent: 'center', borderRadius: 16, backgroundColor: '#62E6B1', paddingHorizontal: 20, marginTop: 20 }, primaryText: { color: '#071310', fontWeight: '900' },
});
