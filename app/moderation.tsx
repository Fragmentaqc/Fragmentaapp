...
export default function ModerationScreen() {
  ...
  if (loading) return <SafeAreaView style={styles.safe}><View style={styles.center}><ActivityIndicator color="#C99A2E" size="large" /></View></SafeAreaView>;
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
    ...
    {openReports.length ? openReports.map((report) => <View key={report.id} style={styles.card}>
      ...
      <TextInput value={notes[report.id] ?? ''} onChangeText={(value) => setNotes((current) => ({ ...current, [report.id]: value }))} style={styles.noteInput} maxLength={1000} placeholder="Note interne facultative" placeholderTextColor="#9FB2AD" />
    ...
    {verificationRequests.length ? verificationRequests.map((request) => <View key={request.id} style={styles.card}>
      ...
      <TextInput value={verificationNotes[request.id] ?? ''} onChangeText={(value) => setVerificationNotes((current) => ({ ...current, [request.id]: value }))} style={styles.noteInput} maxLength={1000} placeholder="Note de décision (obligatoire pour refuser)" placeholderTextColor="#9FB2AD" />
...
const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: '#071A1C' }, center: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24 }, container: { padding: 20, paddingBottom: 70 }, back: { color: '#C99A2E', fontSize: 15, fontWeight: '800', marginBottom: 22 }, eyebrow: { color: '#D8B65A', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 }, title: { color: '#F4EBD8', fontSize: 28, fontWeight: '900', marginTop: 6 }, subtitle: { color: '#C9D6D1', fontSize: 13, marginTop: 7, marginBottom: 14 }, empty: { color: '#B8C8C2', fontSize: 13, lineHeight: 20, textAlign: 'center', marginTop: 18 },
  card: { borderRadius: 0, borderWidth: 1, borderColor: '#3C7475', backgroundColor: '#10363A', padding: 16, marginTop: 12 }, cardTop: { flexDirection: 'row', justifyContent: 'space-between', gap: 10 }, reason: { flex: 1, color: '#F4EBD8', fontSize: 15, fontWeight: '900' }, status: { color: '#071A1C', backgroundColor: '#D8B65A', borderRadius: 0, paddingHorizontal: 8, paddingVertical: 5, fontSize: 8, fontWeight: '900' }, date: { color: '#9FB2AD', fontSize: 10, marginTop: 7 }, details: { color: '#E6E2D5', fontSize: 13, lineHeight: 20, marginTop: 12 }, noDetails: { color: '#9FB2AD', fontSize: 11, fontStyle: 'italic', marginTop: 12 }, target: { borderRadius: 0, backgroundColor: '#1D5A5E', padding: 12, marginTop: 13 }, targetText: { color: '#C99A2E', fontSize: 11, fontWeight: '900' }, noteInput: { minHeight: 46, borderRadius: 0, borderWidth: 1, borderColor: '#3C7475', color: '#F4EBD8', padding: 12, marginTop: 10 }, actions: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 12 }, review: { borderRadius: 0, backgroundColor: '#4B8180', padding: 10 }, reviewText: { color: '#FFF', fontSize: 10, fontWeight: '900' }, resolve: { borderRadius: 0, backgroundColor: '#C99A2E', padding: 10 }, resolveText: { color: '#071A1C', fontSize: 10, fontWeight: '900' }, dismiss: { borderRadius: 0, borderWidth: 1, borderColor: '#7B3535', padding: 10 }, dismissText: { color: '#FFB8B8', fontSize: 10, fontWeight: '900' }, historyTitle: { color: '#F4EBD8', fontSize: 20, fontWeight: '900', marginTop: 30 }, logCard: { borderLeftWidth: 2, borderLeftColor: '#C99A2E', paddingLeft: 13, paddingVertical: 8, marginTop: 8 }, logChange: { color: '#FFF6E5', fontSize: 12, fontWeight: '900' }, logNote: { color: '#B8C8C2', fontSize: 11, lineHeight: 17, marginTop: 5 }, primary: { minHeight: 50, justifyContent: 'center', borderRadius: 0, backgroundColor: '#C99A2E', paddingHorizontal: 20, marginTop: 20 }, primaryText: { color: '#071A1C', fontWeight: '900' },
});
