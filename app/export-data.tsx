...
export default function ExportDataScreen() {
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
    ...
    <Pressable style={[styles.button, exporting && styles.disabled]} onPress={() => void exportData()} disabled={exporting}>{exporting ? <ActivityIndicator color="#0B1710" /> : <Text style={styles.buttonText}>Créer mon export JSON</Text>}</Pressable>
  </ScrollView></SafeAreaView>;
}

const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#0B1710' }, container: { padding: 20, paddingBottom: 70 }, back: { color: '#B86F4B', fontSize: 15, fontWeight: '800', marginBottom: 24 }, eyebrow: { color: '#B86F4B', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 }, title: { color: '#F4E9D6', fontSize: 28, fontWeight: '900', marginTop: 6 }, description: { color: '#CBD5C8', fontSize: 13, lineHeight: 20, marginTop: 10 }, card: { borderRadius: 0, borderWidth: 1, borderColor: '#55775B', backgroundColor: '#173523', padding: 17, marginTop: 23 }, cardTitle: { color: '#FBF1DF', fontSize: 15, fontWeight: '900', marginBottom: 8 }, item: { color: '#AAB6A6', fontSize: 12, lineHeight: 22 }, notice: { color: '#C58A62', fontSize: 11, lineHeight: 18, marginTop: 18 }, button: { minHeight: 58, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#B86F4B', marginTop: 22 }, buttonText: { color: '#0B1710', fontSize: 14, fontWeight: '900' }, disabled: { opacity: 0.5 } });
