...
export default function ExportDataScreen() {
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
    ...
    <Pressable style={[styles.button, exporting && styles.disabled]} onPress={() => void exportData()} disabled={exporting}>{exporting ? <ActivityIndicator color="#071A1C" /> : <Text style={styles.buttonText}>Créer mon export JSON</Text>}</Pressable>
  </ScrollView></SafeAreaView>;
}

const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#071A1C' }, container: { padding: 20, paddingBottom: 70 }, back: { color: '#C99A2E', fontSize: 15, fontWeight: '800', marginBottom: 24 }, eyebrow: { color: '#C99A2E', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 }, title: { color: '#F4EBD8', fontSize: 28, fontWeight: '900', marginTop: 6 }, description: { color: '#C9D6D1', fontSize: 13, lineHeight: 20, marginTop: 10 }, card: { borderRadius: 0, borderWidth: 1, borderColor: '#3C7475', backgroundColor: '#10363A', padding: 17, marginTop: 23 }, cardTitle: { color: '#FFF6E5', fontSize: 15, fontWeight: '900', marginBottom: 8 }, item: { color: '#A2B3AB', fontSize: 12, lineHeight: 22 }, notice: { color: '#D8B65A', fontSize: 11, lineHeight: 18, marginTop: 18 }, button: { minHeight: 58, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#C99A2E', marginTop: 22 }, buttonText: { color: '#071A1C', fontSize: 14, fontWeight: '900' }, disabled: { opacity: 0.5 } });
