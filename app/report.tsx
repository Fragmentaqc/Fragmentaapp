...
export default function ReportScreen() {
  ...
  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
    ...
    <Text style={styles.label}>Précisions facultatives</Text><TextInput value={details} onChangeText={setDetails} style={styles.area} multiline textAlignVertical="top" maxLength={1000} placeholder="Décris brièvement le problème…" placeholderTextColor="#B8B59E" />
    ...
    <Pressable style={styles.submit} onPress={() => void submit()} disabled={sending}>{sending ? <ActivityIndicator color="#173E28" /> : <Text style={styles.submitText}>Envoyer le signalement</Text>}</Pressable>
  </ScrollView></KeyboardAvoidingView></SafeAreaView>;
}

const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: '#173E28' }, flex: { flex: 1 }, container: { padding: 20, paddingBottom: 70 }, center: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24 },
  back: { color: '#E9576F', fontSize: 15, fontWeight: '800', marginBottom: 22 }, eyebrow: { color: '#F0A36B', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 }, title: { color: '#F5E6C8', fontSize: 28, fontWeight: '900', marginTop: 6 }, subtitle: { color: '#D8CFBA', fontSize: 13, lineHeight: 20, marginTop: 8 }, label: { color: '#FFF1D6', fontSize: 13, fontWeight: '900', marginTop: 22, marginBottom: 9 },
  reasons: { gap: 8 }, reason: { minHeight: 50, flexDirection: 'row', alignItems: 'center', borderRadius: 0, borderWidth: 1, borderColor: '#356F43', backgroundColor: '#245A35', paddingHorizontal: 14 }, reasonActive: { borderColor: '#E9576F', backgroundColor: '#3B7C49' }, dot: { width: 12, height: 12, borderRadius: 0, borderWidth: 2, borderColor: '#BDB7A3', marginRight: 11 }, dotActive: { borderColor: '#E9576F', backgroundColor: '#E9576F' }, reasonText: { color: '#FFF1D6', fontSize: 13, fontWeight: '800' },
  area: { minHeight: 140, borderRadius: 0, borderWidth: 1, borderColor: '#356F43', backgroundColor: '#245A35', color: '#F5E6C8', padding: 15 }, count: { color: '#B8B59E', fontSize: 10, textAlign: 'right', marginTop: 5 }, notice: { borderRadius: 0, backgroundColor: '#2F6F3E', padding: 14, marginTop: 18 }, noticeText: { color: '#D0C4A9', fontSize: 11, lineHeight: 17 }, submit: { minHeight: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#F0A36B', paddingHorizontal: 22, marginTop: 22 }, submitText: { color: '#173E28', fontSize: 14, fontWeight: '900' },
});
