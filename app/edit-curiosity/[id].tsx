  ...
  return (
    <SafeAreaView style={styles.safeArea}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <Pressable style={styles.saveButton} onPress={() => void save()} disabled={saving}>{saving ? <ActivityIndicator color="#173E28" /> : <Text style={styles.saveText}>Enregistrer les changements</Text>}</Pressable>
      </ScrollView>
...
const styles = StyleSheet.create({
  flex: { flex: 1 }, safeArea: { flex: 1, backgroundColor: '#173E28' }, center: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24 }, container: { padding: 20, paddingBottom: 70 }, header: { minHeight: 48, justifyContent: 'center', marginBottom: 18 }, back: { color: '#E9576F', fontSize: 15, fontWeight: '800' }, title: { position: 'absolute', alignSelf: 'center', color: '#F5E6C8', fontSize: 19, fontWeight: '900' }, errorTitle: { color: '#F5E6C8', fontSize: 19, fontWeight: '900' },
  label: { color: '#FFF1D6', fontSize: 13, fontWeight: '800', marginTop: 16, marginBottom: 8 }, input: { minHeight: 54, borderRadius: 0, borderWidth: 1, borderColor: '#356F43', backgroundColor: '#245A35', color: '#F5E6C8', paddingHorizontal: 15, paddingVertical: 13 }, textarea: { minHeight: 140 }, chip: { borderRadius: 0, backgroundColor: '#2F6F3E', paddingHorizontal: 14, paddingVertical: 10, marginRight: 8 }, active: { backgroundColor: '#6D9F6B' }, chipText: { color: '#FFF1D6', fontSize: 12, fontWeight: '800' }, row: { flexDirection: 'row', gap: 9 }, option: { flex: 1, alignItems: 'center', borderRadius: 0, borderWidth: 1, borderColor: '#5B8F5D', padding: 14 }, note: { color: '#D0C4A9', fontSize: 12, marginTop: 20 }, saveButton: { minHeight: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#E9576F', paddingHorizontal: 20, marginTop: 24 }, saveText: { color: '#173E28', fontSize: 14, fontWeight: '900' },
});
