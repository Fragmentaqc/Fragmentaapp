  ...
  return (
    <SafeAreaView style={styles.safeArea}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <Pressable style={styles.saveButton} onPress={() => void save()} disabled={saving}>{saving ? <ActivityIndicator color="#0B1710" /> : <Text style={styles.saveText}>Enregistrer les changements</Text>}</Pressable>
      </ScrollView>
...
const styles = StyleSheet.create({
  flex: { flex: 1 }, safeArea: { flex: 1, backgroundColor: '#0B1710' }, center: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24 }, container: { padding: 20, paddingBottom: 70 }, header: { minHeight: 48, justifyContent: 'center', marginBottom: 18 }, back: { color: '#B86F4B', fontSize: 15, fontWeight: '800' }, title: { position: 'absolute', alignSelf: 'center', color: '#F4E9D6', fontSize: 19, fontWeight: '900' }, errorTitle: { color: '#F4E9D6', fontSize: 19, fontWeight: '900' },
  label: { color: '#FBF1DF', fontSize: 13, fontWeight: '800', marginTop: 16, marginBottom: 8 }, input: { minHeight: 54, borderRadius: 0, borderWidth: 1, borderColor: '#3D6648', backgroundColor: '#173523', color: '#F4E9D6', paddingHorizontal: 15, paddingVertical: 13 }, textarea: { minHeight: 140 }, chip: { borderRadius: 0, backgroundColor: '#21472F', paddingHorizontal: 14, paddingVertical: 10, marginRight: 8 }, active: { backgroundColor: '#6F8D6C' }, chipText: { color: '#FBF1DF', fontSize: 12, fontWeight: '800' }, row: { flexDirection: 'row', gap: 9 }, option: { flex: 1, alignItems: 'center', borderRadius: 0, borderWidth: 1, borderColor: '#55775B', padding: 14 }, note: { color: '#BCC8B8', fontSize: 12, marginTop: 20 }, saveButton: { minHeight: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#B86F4B', paddingHorizontal: 20, marginTop: 24 }, saveText: { color: '#0B1710', fontSize: 14, fontWeight: '900' },
});
