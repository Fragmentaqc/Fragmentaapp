    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <TextInput style={styles.input} value={title} onChangeText={setTitle} maxLength={80} placeholder="Ex. Premier sommet atteint" placeholderTextColor="#A8B3A4" editable={!saving} />
          <Text style={styles.label}>Récit</Text>
          <TextInput style={[styles.input, styles.area]} value={body} onChangeText={setBody} maxLength={1000} multiline textAlignVertical="top" placeholder="Raconte ce qui s’est passé…" placeholderTextColor="#A8B3A4" editable={!saving} />
          <Text style={styles.label}>Date (AAAA-MM-JJ)</Text>
          <TextInput style={styles.input} value={date} onChangeText={setDate} maxLength={10} keyboardType="numbers-and-punctuation" placeholder="2026-07-20" placeholderTextColor="#A8B3A4" editable={!saving} />
          <Text style={styles.label}>Position du moment</Text>
          <LocationPicker coordinate={coordinate} onSelect={setCoordinate} />

          <Pressable style={styles.publish} onPress={() => void save('published')} disabled={saving}>{saving ? <ActivityIndicator color="#0B1710" /> : <Text style={styles.publishText}>Publier le fragment</Text>}</Pressable>
...
const styles = StyleSheet.create({
  ...
  back: { color: '#B86F4B', fontSize: 15, fontWeight: '800', marginBottom: 22 }, eyebrow: { color: '#B86F4B', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 },
  title: { color: '#F4E9D6', fontSize: 30, fontWeight: '900', marginTop: 6 }, subtitle: { color: '#CBD5C8', fontSize: 14, marginTop: 7, marginBottom: 20 }, link: { color: '#B86F4B', marginTop: 18 },
  photoButton: { height: 150, alignItems: 'center', justifyContent: 'center', borderRadius: 0, borderWidth: 1, borderStyle: 'dashed', borderColor: '#748D73', backgroundColor: '#173523' },
  photoIcon: { color: '#B86F4B', fontSize: 35 }, photoTitle: { color: '#F4E9D6', fontWeight: '800' }, helper: { color: '#7F968B', fontSize: 11, marginTop: 5 }, images: { gap: 10, paddingVertical: 14 },
  ...
  label: { color: '#FBF1DF', fontSize: 14, fontWeight: '800', marginTop: 18, marginBottom: 8 }, input: { minHeight: 54, borderRadius: 0, borderWidth: 1, borderColor: '#3D6648', backgroundColor: '#173523', color: '#F4E9D6', padding: 15 }, area: { minHeight: 150 },
  publish: { minHeight: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#B86F4B', marginTop: 26 }, publishText: { color: '#0B1710', fontWeight: '900', fontSize: 15 },
