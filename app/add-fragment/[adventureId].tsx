    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          <TextInput style={styles.input} value={title} onChangeText={setTitle} maxLength={80} placeholder="Ex. Premier sommet atteint" placeholderTextColor="#9FB2AD" editable={!saving} />
          <Text style={styles.label}>Récit</Text>
          <TextInput style={[styles.input, styles.area]} value={body} onChangeText={setBody} maxLength={1000} multiline textAlignVertical="top" placeholder="Raconte ce qui s’est passé…" placeholderTextColor="#9FB2AD" editable={!saving} />
          <Text style={styles.label}>Date (AAAA-MM-JJ)</Text>
          <TextInput style={styles.input} value={date} onChangeText={setDate} maxLength={10} keyboardType="numbers-and-punctuation" placeholder="2026-07-20" placeholderTextColor="#9FB2AD" editable={!saving} />
          <Text style={styles.label}>Position du moment</Text>
          <LocationPicker coordinate={coordinate} onSelect={setCoordinate} />

          <Pressable style={styles.publish} onPress={() => void save('published')} disabled={saving}>{saving ? <ActivityIndicator color="#071A1C" /> : <Text style={styles.publishText}>Publier le fragment</Text>}</Pressable>
...
const styles = StyleSheet.create({
  safe: { flex: 1, backgroundColor: '#071A1C' }, flex: { flex: 1 }, container: { padding: 18, paddingBottom: 70 }, center: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  back: { color: '#C99A2E', fontSize: 15, fontWeight: '800', marginBottom: 22 }, eyebrow: { color: '#C99A2E', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 },
  title: { color: '#F4EBD8', fontSize: 30, fontWeight: '900', marginTop: 6 }, subtitle: { color: '#C9D6D1', fontSize: 14, marginTop: 7, marginBottom: 20 }, link: { color: '#C99A2E', marginTop: 18 },
  photoButton: { height: 150, alignItems: 'center', justifyContent: 'center', borderRadius: 0, borderWidth: 1, borderStyle: 'dashed', borderColor: '#6A9692', backgroundColor: '#10363A' },
  photoIcon: { color: '#C99A2E', fontSize: 35 }, photoTitle: { color: '#F4EBD8', fontWeight: '800' }, helper: { color: '#7F968B', fontSize: 11, marginTop: 5 }, images: { gap: 10, paddingVertical: 14 },
  ...
  label: { color: '#FFF6E5', fontSize: 14, fontWeight: '800', marginTop: 18, marginBottom: 8 }, input: { minHeight: 54, borderRadius: 0, borderWidth: 1, borderColor: '#265F63', backgroundColor: '#10363A', color: '#F4EBD8', padding: 15 }, area: { minHeight: 150 },
  publish: { minHeight: 56, alignItems: 'center', justifyContent: 'center', borderRadius: 0, backgroundColor: '#C99A2E', marginTop: 26 }, publishText: { color: '#071A1C', fontWeight: '900', fontSize: 15 },
